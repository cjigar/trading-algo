...
- [ ] 6.2 Manually verify against the live snapshot data that trends match hand-computed OI diffs for a sample strike across the four windows. _(Deploy-gated: needs live snapshots on the server.)_
- [ ] 6.3 Deploy during a market-closed window; if the Postgres `option_chain_snapshots` table is large, pre-create the composite index `CONCURRENTLY` before the code lands (per Migration Plan). _(Held: market open at implementation time — do after 15:30 IST.)_
- [ ] 6.4 After deploy, confirm the dashboard chain shows live arrows and unavailable windows fill in as the session progresses. _(Deploy-gated.)_
