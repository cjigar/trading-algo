...
### D2: TOTP-first auth with daily re-login
`SessionManager` performs `totp_login(mobile, ucc, totp)` → `totp_validate(mpin)`, generating the TOTP with `pyotp` from a stored secret. It schedules a pre-market re-login (~08:30–09:00 IST) and re-authenticates on mid-session token-expiry errors. **Alternative:** manual OTP entry each session — rejected because it defeats unattended operation.
