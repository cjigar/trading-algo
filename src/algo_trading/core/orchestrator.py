...
class Orchestrator:
    ...
    def refresh_india_vix(self) -> bool:
        """Fetch India VIX from NSE and publish it to the rate ticker. Kotak does not carry India
        VIX, so it comes from NSE's public allIndices API. Display-only and fail-soft: a failed or
        disabled fetch leaves the last value to age to stale and never disturbs the trading loop.

        Writes two ``index_spots`` rows under ``INDIAVIX``: today's ``last`` and a prior-day row
        (NSE's ``previousDay``) holding ``previousClose`` — so the existing day-change machinery
        renders VIX with the correct change, no ticker/schema changes needed. Returns True on write.
        """
        if self._settings.india_vix_source != "nse":
            return False
        from algo_trading.feed.india_vix import fetch_india_vix

        quote = fetch_india_vix()
        if quote is None:
            return False
        # Prior-day baseline first (dated NSE's previous trading day) so the change is measured
        # against the real close; then today's live level.
        if quote.prev_day is not None:
            self._repo.upsert_index_spots({"INDIAVIX": quote.prev_close}, trading_day=quote.prev_day)
        self._repo.upsert_index_spots({"INDIAVIX": quote.last})
        return True
